...
app.use('/uploads', express.static(path.join(__dirname,'..', 'Backend-CRM-Download', 'files', 'Contrats')));

// handle storage using multer
var storage = multer.diskStorage({
  destination: function (req, file, cb) {
    cb(null, path.join(__dirname,'..', 'Backend-CRM-Download', 'files', 'Contrats'));
  },
  filename: function (req, file, cb) {
    const originalname = file.originalname;
    const extension = path.extname(originalname);
    const filename = `contrat-${originalname}`;
    cb(null, `${filename}`);
  }
});
