// Chakra imports
import { Avatar, Box, Button, Flex, FormControl, FormErrorMessage, FormLabel, Grid, Icon, Input, Stack, Tab, TabList, TabPanel, TabPanels, Tabs, Text, useColorModeValue } from '@chakra-ui/react';
...
export default function Overview() {
  ...
  const isErrornumFiscal = numFiscal === '';
  const isErrorca = ca === '';
  const isErrordomaine = domaine === '';
  const isErrordateCreation = dateCreation === '';
  ...
  useEffect(() => {
    fetch('http://localhost:8080/api/role_entreprise/entreprisePerProp?id=' + localStorage.getItem("user"))
      ...
      .then(data => {
        console.log(data)
        if (data.idUser != null) { setHasCompany(true) }
      })
  }, []);

  const EntrepriseFetch = async () => {
    try {
      if (HasCompany) { alert("Vous avez deja une entreprise") }
      else {
        console.log("test 1")
        const result = await axios.post("http://localhost:8080/api/auth/ajoutEntreprise",
          { mail, numTel, nomEntreprise, numFiscal, description, ca, domaine, dateCreation, adresse });
        console.log(result);
        console.log(result.data.idUser);
        const result2 = await axios.put("http://localhost:8080/api/auth/ajoutRole_entreprise",
          { idEntreprise: result.data.idUser, roleContact: "ROLE_PROPRIETAIRE", idContact: localStorage.getItem("user") });
        console.log(result2);
        history.push("/profile/profile-company?id=" + result.data.idUser);
      }
  ...
  async function testentreprise() {
    if (isErrormail === true || mail.includes("@") === false) { alert("mail Invalid"); }
    else if (isErrornumTel === true) { alert("numero tel invalid"); }
    else if (isErrornomEntreprise === true) { alert("nomEntreprise invalid"); }
    else if (isErrordomaine === true) { alert("domaine invalid"); }
    else { EntrepriseFetch() }
  }
  ...
  return (
    <Box pt={{ base: '130px', md: '80px', xl: '80px' }}>
      ...
      <Grid
        ...
        <Flex gridArea='1 / 1 / 4 / 4' minH='365px' pe='20px'
                  ...
                  <Flex mb="40px">
                    ...
                    >
                      ...
                      <Text color="gray.400" fontWeight="normal" fontSize="sm">
                        Faites-nous savoir le nom et l'adresse e-mail. Utilisez une adresse que
                        vous ne dérangez pas quand d'autres utilisateurs vous contactent à
                      </Text>
                    </Flex>
                  </Flex>
                  <Box>
                    <Flex direction="column" w="100%">
                      ...
                      >
                        <Box
                          ...
                          mb={{ sm: "25px" }}
                        >
                          <Avatar src={require("assets/img/avatars/company.png")} w="100%" h="100%" />
                        </Box>
                        <Stack direction="column" spacing="20px" w="100%">
                          <FormControl isInvalid={isErrornomEntreprise}>
                            ...
                            {!isErrornomEntreprise ? (
                              <FormErrorMessage>Un Nom de Entreprise est requis.</FormErrorMessage>
                            ) : (<FormErrorMessage>Un Nom de Entreprise est requis.</FormErrorMessage>
                            )}
                          </FormControl>
                          <FormControl isInvalid={isErrornumTel}>
                            ...
                            />
                            {!isErrornumTel ? (
                              <FormErrorMessage>Un Numero de Tel est requis.</FormErrorMessage>
                            ) : (<FormErrorMessage>Un Numero de Tel est requis.</FormErrorMessage>
                            )}
                          </FormControl>
                          ...
                          <FormControl isInvalid={isErroradresse}>
                            ...
                            />
                            {!isErroradresse ? (
                              <FormErrorMessage>Une Adresse est requis.</FormErrorMessage>
                            ) : (<FormErrorMessage>Une Adresse est requis.</FormErrorMessage>
                            )}
                          </FormControl>
                        </Stack>
                    ...
                    <Flex direction="column" w="100%">
                      ...
                      >
                        <FormControl isInvalid={isErrordomaine}>
                          ...
                          />
                          {!isErrordomaine ? (
                            <FormErrorMessage>Un Domaine est requis.</FormErrorMessage>
                          ) : (<FormErrorMessage>Un Domaine est requis.</FormErrorMessage>
                          )}
                        </FormControl>
                      </Stack>
                  ...
                  <Box>
                    <Flex direction="column" w="100%">
                      <Stack direction="column" spacing="20px">
                        <FormControl isInvalid={isErrornumFiscal}>
                          <FormLabel
                            color={textColor}
                            fontWeight="bold"
                            fontSize="xs"
                          ...
                          />
                          {!isErrornumFiscal ? (
                            <FormErrorMessage>Un numéro d'identification fiscale est requis.</FormErrorMessage>
                          ) : (<FormErrorMessage>Un numéro d'identification fiscale est requis.</FormErrorMessage>
                          )}
                        </FormControl>
                        <FormControl isInvalid={isErrorca}>
                          <FormLabel
                            color={textColor}
                            fontWeight="bold"
                            fontSize="xs"
                          ...
                          />
                          {!isErrorca ? (
                            <FormErrorMessage>Un Chiffre d'affaires est requis.</FormErrorMessage>
                          ) : (<FormErrorMessage>Un Chiffre d'affaires est requis.</FormErrorMessage>
                          )}
                        </FormControl>
                        <FormControl gridColumn={{ sm: "1 / 3", lg: "auto" }} isInvalid={isErrordateCreation}>
                          <FormLabel
                            color={textColor}
                            fontWeight="bold"
                            fontSize="xs"
                          ...
                          />
                          {!isErrordateCreation ? (
                            <FormErrorMessage>Un Date de Lancement est requis.</FormErrorMessage>
                          ) : (<FormErrorMessage>Un Date de Lancement est requis.</FormErrorMessage>
                          )}
                        </FormControl>
                        <FormControl>
